...
// LoadConfig loads config from file (YAML/JSON) and merges with inline flags.
func LoadConfig(configPath string, inlineMaps []string, inputDir string) (*Config, error) {
	cfg := &Config{}
...
// LoadConfigWithNoConfig loads config, but if noConfig is true, ignores all config files and uses only CLI flags.
func LoadConfigWithNoConfig(configPath string, inlineMaps []string, inputDir string, noConfig bool) (*Config, error) {
	cfg := &Config{}
